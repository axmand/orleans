...
namespace Engine.NLP.Forms
{
    ...
    public partial class NLPExpertiseForm : Form
    {
        ...
        void InitialConfigValue()
        {
            ...
            hazard = NLPConfiguration.HazardString.Split(';').Where(p => p.Length > 0).ToList();
            exposure = NLPConfiguration.ExposureString.Split(';').Where(p => p.Length > 0).ToList();
            humanBehavior = NLPConfiguration.HumanBehaviorString.Split(';').Where(p => p.Length > 0).ToList();
            //ui
        ...
        private void UpdateProcessTip(double process, double[][] a = null, double[][] b = null, double[][] c = null)
        {
            if (process == 1.0)
            {
                ...
                NLPScottPlotForm scott_plot_form = new NLPScottPlotForm();
        ...
        private void Visual_button_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    //0.通知预处理开始
                    Invoke(new UpdateProcessTipHandler(UpdateProcessTip), 0.0, null, null, null);
                    int totalNum = hazard.Count + exposure.Count + humanBehavior.Count;
                    //1. 构建词W集合
                    float[][] words = new float[totalNum][];
                    //2.定义词颜色
                    for (int i = 0; i < hazard.Count; i++)
                        words[i] = GloveNet.ToFloat(hazard[i]);
                    for (int i = 0; i < exposure.Count; i++)
                        words[hazard.Count + i] = GloveNet.ToFloat(exposure[i]);
                    for (int i = 0; i < humanBehavior.Count; i++)
                        words[hazard.Count + exposure.Count + i] = GloveNet.ToFloat(humanBehavior[i]);
                    //3.t-SNE算法降维
                    if (words.GetLength(0) > 0)
                    {
                        ...
                        MessageBox.Show("缺少词汇，请检查知识库后重新操作", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                })
                {
                    IsBackground = true
                };
        ...
        {
            NLPConfiguration.HumanBehaviorString = string.Join(";", humanBehavior.ToArray());
            NLPConfiguration.ExposureString = string.Join(";", exposure.ToArray());
            NLPConfiguration.HazardString = string.Join(";", hazard.ToArray());
