...
namespace Engine.GIS.GEntity
{
    ...
    public class GBitmap
    {
        ...
        /// 将一个字节数组转换为24位真彩色图
        /// </summary>
        /// <param name="normalbuffer">字节数组</param>
        ...
        public static Bitmap ToGrayBitmap(float[,] normalbuffer, int width, int height)
        {
            ...
            int index = 0;
            int imgwidth = normalbuffer.GetLength(0);
            int imgheight = normalbuffer.GetLength(1);
            for (int i = 0; i < imgheight; i++)
                for (int j = 0; j < imgwidth; j++)
                    rawValues[index++] = double.IsNaN(normalbuffer[j, i]) ? (byte)0 : Convert.ToByte(normalbuffer[j, i] * 255);
            //申请目标位图的变量，并将其内存区域锁定
        ...
        /// <summary>
        /// 用于在空间中显示的图片，并不需要1:1处理
        /// </summary>
        /// <param name="bytebuffer"></param>
        /// <param name="width"></param>
        /// <param name="height"></param>
        /// <returns></returns>
        public static Bitmap ToGrayBitmap(byte[,] bytebuffer, int width, int height)
        {
            //将用户指定的imageArray二维数组转换为一维数组rawValues
            byte[] rawValues = new byte[width * height];
            int index = 0;
            int imgwidth = bytebuffer.GetLength(0);
            int imgheight = bytebuffer.GetLength(1);
            for (int i = 0; i < imgheight; i++)
                for (int j = 0; j < imgwidth; j++)
                    rawValues[index++] = double.IsNaN(bytebuffer[j, i]) ? (byte)0 : bytebuffer[j,i];
            //申请目标位图的变量，并将其内存区域锁定
            Bitmap bmp = new Bitmap(width, height, PixelFormat.Format24bppRgb);
            BitmapData bmpData = bmp.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.WriteOnly, PixelFormat.Format24bppRgb);
            //获得图像的参数
