...
namespace Engine.Brain.Method.DeepQNet.Env
{
    ...
    public class ImageClassifyEnv : IEnv
    {
        ...
        public ImageClassifyEnv(GRasterLayer featureRasterLayer, GRasterLayer labelRasterLayer , int sampleSizeLimit = 200, bool lerpPick = true)
        {
             ...
             * 2.背景值设置为 0 
             * 依据灰度直方图，统计非0区域 有值类型
             * modify 2019/11/1 保留“背景值设置为0”
             */
           using(IRasterBandStatisticTool pRasterBandStatisticTool = new GRasterBandStatisticTool())
            {
                pRasterBandStatisticTool.Visit(_labelRasterLayer.BandCollection[0]);
                ActionNum = pRasterBandStatisticTool.StaisticalRawPixelGraph.Where(o=> o.Key != 0.0 && o.Value > 0).Count();
            }
            Prepare();
        }
        ...
        public (float[] state, float reward) Step(float[] action)
        {
            if (action == null)
            {
                var (_c_x, _c_y, _) = (_current_x, _current_y, _current_classindex);
                (_current_x, _current_y, _current_classindex) = RandomAccessMemory();
            ...
            else
            {
                ...
                //float reward = NP.Argmax(action) == NP.Argmax(_current_classindex) ? 1.0f : 0.0f;
                (_current_x, _current_y, _current_classindex) = RandomAccessMemory();
